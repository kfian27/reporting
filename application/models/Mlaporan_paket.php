<?php
	class Mlaporan_paket extends CI_Model {
    	...
    	function get_tanggal($tgl1 ='',$tgl2 ='',$param1=''){
        	$sql = "SELECT	NM_HEADER,	COUNT (NM_HEADER) AS JUMLAH FROM ( SELECT DISTINCT T_FO.TGL_REGISTRASI,	T_FO.NO_REGISTRASI,
			SUBSTR(DATA_PEMOHON.KD_IJIN,1,4) AS KD_HEADER,	MIJIN.NM_HEADER	FROM T_FO, DATA_PEMOHON, MIJIN,	M_PAKET_DETAIL WHERE
			T_FO.NO_REGISTRASI = DATA_PEMOHON.NO_REGISTRASI	AND TO_CHAR ( DATA_PEMOHON.TGL_REGISTRASI, 'ddmmyyyy') = TO_CHAR (
			T_FO.TGL_REGISTRASI,'ddmmyyyy') AND SUBSTR (DATA_PEMOHON.KD_IJIN, 1, 4) = SUBSTR (MIJIN.KD_IJIN, 1, 4) AND T_FO.ID_PAKET = '$param1' AND T_FO.TGL_REGISTRASI BETWEEN TO_DATE ('$tgl1', 'yyyy-mm-dd') AND TO_DATE ('$tgl2', 'yyyy-mm-dd')	AND ID_ALUR_PROSES != 100) WHERE NM_HEADER NOT LIKE ' ' GROUP BY NM_HEADER ORDER BY NM_HEADER";

			$query = $this->db->query($sql);
			return $query->result();
        }
        function get_tanggal_imb($tgl1 ='',$tgl2 ='',$param1=''){
        	$sql = "SELECT NAMA_IJIN, COUNT (NAMA_IJIN) AS JUMLAHNYA FROM(SELECT T_FO.TGL_REGISTRASI, T_FO.NO_REGISTRASI,T_FO_SUB_IJIN.KD_IJIN, MIJIN.NAMA_IJIN FROM T_FO,T_FO_SUB_IJIN,MIJIN,M_PAKET_DETAIL WHERE T_FO.NO_REGISTRASI = T_FO_SUB_IJIN.NO_REGISTRASI AND T_FO.TGL_REGISTRASI = T_FO_SUB_IJIN.TGL_REGISTRASI AND T_FO_SUB_IJIN.KD_IJIN = MIJIN.KD_IJIN AND substr(T_FO_SUB_IJIN.KD_IJIN1,1,4) = M_PAKET_DETAIL.KD_IJIN_HEADER AND T_FO.TGL_REGISTRASI BETWEEN TO_DATE ('$tgl1', 'yyyy-mm-dd') AND TO_DATE ('$tgl2', 'yyyy-mm-dd') AND T_FO.ID_PAKET ='$param1') GROUP BY NAMA_IJIN ORDER BY NAMA_IJIN ASC";
        ...
        function get_tanggal_selesai($tgl1 ='',$tgl2 ='',$param1='',$param2=''){
        	$sql = "SELECT NM_HEADER, COUNT (NM_HEADER) AS SELESAI FROM ( SELECT DISTINCT	T_FO.TGL_REGISTRASI,T_FO.NO_REGISTRASI,
			SUBSTR (DATA_PEMOHON.KD_IJIN, 1, 4) AS KD_HEADER, MIJIN.NM_HEADER FROM T_FO, DATA_PEMOHON, MIJIN, M_PAKET_DETAIL WHERE
			T_FO.NO_REGISTRASI = DATA_PEMOHON.NO_REGISTRASI	AND TO_CHAR (DATA_PEMOHON.TGL_REGISTRASI,'ddmmyyyy') = TO_CHAR (T_FO.TGL_REGISTRASI,'ddmmyyyy')	AND SUBSTR (DATA_PEMOHON.KD_IJIN, 1, 4) = SUBSTR (MIJIN.KD_IJIN, 1, 4) AND T_FO.ID_PAKET = '$param1' AND T_FO.TGL_REGISTRASI BETWEEN TO_DATE ('$tgl1', 'yyyy-mm-dd') AND TO_DATE ('$tgl2', 'yyyy-mm-dd') AND ID_ALUR_PROSES IN (5, 6) and NM_HEADER like '%$param2%' AND DATA_PEMOHON.STS_TOLAK IS NULL) GROUP BY NM_HEADER";

			$query = $this->db->query($sql);
			return $query->result();
        }
        function get_tanggal_proses($tgl1 ='',$tgl2 ='',$param1='',$param2=''){
        	$sql = "SELECT NM_HEADER, COUNT (NM_HEADER) AS PROSES FROM ( SELECT DISTINCT	T_FO.TGL_REGISTRASI,T_FO.NO_REGISTRASI,
			SUBSTR (DATA_PEMOHON.KD_IJIN, 1, 4) AS KD_HEADER, MIJIN.NM_HEADER FROM T_FO, DATA_PEMOHON, MIJIN, M_PAKET_DETAIL WHERE
			T_FO.NO_REGISTRASI = DATA_PEMOHON.NO_REGISTRASI	AND TO_CHAR (DATA_PEMOHON.TGL_REGISTRASI,'ddmmyyyy') = TO_CHAR (T_FO.TGL_REGISTRASI,'ddmmyyyy')	AND SUBSTR (DATA_PEMOHON.KD_IJIN, 1, 4) = SUBSTR (MIJIN.KD_IJIN, 1, 4) AND T_FO.ID_PAKET = '$param1' AND T_FO.TGL_REGISTRASI BETWEEN TO_DATE ('$tgl1', 'yyyy-mm-dd') AND TO_DATE ('$tgl2', 'yyyy-mm-dd') AND ID_ALUR_PROSES NOT IN (5, 6, 9, 100) and NM_HEADER like '%$param2%' AND DATA_PEMOHON.STS_TOLAK IS NULL) GROUP BY NM_HEADER";

			$query = $this->db->query($sql);
			return $query->result();
        }
        function get_tanggal_pending($tgl1 ='',$tgl2 ='',$param1='',$param2=''){
        	$sql = "SELECT NM_HEADER, COUNT (NM_HEADER) AS PENDING FROM ( SELECT DISTINCT	T_FO.TGL_REGISTRASI,T_FO.NO_REGISTRASI,
			SUBSTR (DATA_PEMOHON.KD_IJIN, 1, 4) AS KD_HEADER, MIJIN.NM_HEADER FROM T_FO, DATA_PEMOHON, MIJIN, M_PAKET_DETAIL WHERE
			T_FO.NO_REGISTRASI = DATA_PEMOHON.NO_REGISTRASI	AND TO_CHAR (DATA_PEMOHON.TGL_REGISTRASI,'ddmmyyyy') = TO_CHAR (T_FO.TGL_REGISTRASI,'ddmmyyyy')	AND SUBSTR (DATA_PEMOHON.KD_IJIN, 1, 4) = SUBSTR (MIJIN.KD_IJIN, 1, 4) AND T_FO.ID_PAKET = '$param1' AND T_FO.TGL_REGISTRASI BETWEEN TO_DATE ('$tgl1', 'yyyy-mm-dd') AND TO_DATE ('$tgl2', 'yyyy-mm-dd') AND ID_ALUR_PROSES IN (9) and NM_HEADER like '%$param2%' AND DATA_PEMOHON.STS_TOLAK IS NULL) GROUP BY NM_HEADER";
